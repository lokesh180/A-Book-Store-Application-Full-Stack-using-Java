...
public class EditBookServlet extends GenericServlet 
{
	...
	throws ServletException,IOException
	{
		...
		try
		{
			...
			RequestDispatcher rd=req.getRequestDispatcher("ebook.html");
			rd.include(req, res);
		}
		catch(Exception e)
		{
		...
		Long bprice=Long.parseLong(req.getParameter("bprice"));
		Integer bqty=Integer.parseInt(req.getParameter("bqty"));
		
		
		UpdateDAO udao=new UpdateDAO();
		int k=udao.update(bcode,bname,bauthor,bprice,bqty);
		if(k>0)
		{
			RequestDispatcher rd=req.getRequestDispatcher("link1.html");
			rd.include(req, res);
			pw.println("<br><br>Books Edited Successfully");
		}
		 }
